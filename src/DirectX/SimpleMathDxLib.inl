...
inline DxLib::Rectangle Rectangle::Intersect(const Rectangle& ra, const Rectangle& rb) noexcept
{
...
inline DxLib::Rectangle Rectangle::Union(const Rectangle& ra, const Rectangle& rb) noexcept
{
    long righta = ra.x + ra.width;
    long rightb = rb.x + rb.width;
...
inline bool Ray::Intersects( const DirectX::BoundingSphere& sphere,
							 _Out_ float& Dist ) const noexcept
{
    return sphere.Intersects(position, direction, Dist);
}

inline bool Ray::Intersects( const DirectX::BoundingBox& box,
							 _Out_ float&				 Dist ) const noexcept
{
